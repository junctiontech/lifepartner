	...
	exit();
		...
		foreach ($ProfileData as $data)
			{	//print_r($data);die;
				...
				$serverProfileId= $data['serverProfileId'];//print_r($serverProfileId);die;
		        $s_no=$data['S_no'];
       		 if ($serverProfileId=='')
        	      {
        		   /************************ check profile category wise not more than 3 ********************************/
					...
					{	
						...
						$tempName =$imagename['imageName'];//echo  $tempName;die;
